...
export async function getOwnerSummary(owner, nowMs = Date.now()) {
  ...
  for (const p of policies) {
    ...
    const status = m?.revoked
      ? 'revoked'
      : m && nowMs >= Number(m.expires_at_ms)
        ? 'expired'
        : 'active';
    positions.push({
...
export async function getBalances(owner) {
  ...
  const price = {
    SUI: Number(market.SUI_DBUSDC?.last_price) || 0,
    USDC: 1,
    DEEP: Number(market.DEEP_DBUSDC?.last_price) || 0,
    WAL: Number(market.WAL_DBUSDC?.last_price) || 0,
  };
  const sym = (ct) =>
    ct.endsWith('::sui::SUI')
      ? ['SUI', 9]
...
export async function readBalanceManagerBalance(
  client,
  coinType,
  sender = DEPLOYMENT.agent.address
) {
...
export async function getActivity(wrapperId, nowMs = Date.now()) {
  ...
  if (!wrapper)
    return { status: 'error', code: 'NOT_FOUND', message: 'Wrapper not found on-chain.' };
