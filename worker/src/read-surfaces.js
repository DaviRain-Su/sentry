...
export function resolveFundingThresholds({ configured = {}, requested = {} } = {}) {
  ...
  for (const asset of Object.keys(DEFAULT_FUNDING_THRESHOLDS)) {
    ...
    out[asset] = {
      ...
      source:
        required === requestedValue && requested[asset] != null
...
export function parseIntentWithStability(
  parseFn,
  cache,
  body,
  nowMs = Date.now(),
  cacheTtlMs = DEFAULT_PARSE_CACHE_TTL_MS
) {
  ...
  if (
    cached &&
    cached.cache_expires_at_ms > nowMs &&
    Number(cached.result?.strategy?.expires_at_ms ?? 0) > nowMs
  ) {
  ...
  if (result.status === 'ok') {
    ...
    cache.set(key, {
      ...
      cache_expires_at_ms: Math.min(
        nowMs + cacheTtlMs,
        Number.isFinite(strategyExpiry) ? strategyExpiry : nowMs
      ),
    });
  }
  return result;
...
export function enrichPolicyFromChain({
  eventPolicy,
  wrapper,
  mandate,
  createdTx,
...
export function buildFundingReadiness({
  ...
  const thresholds = {
    DBUSDC: {
      required: numericString(requiredDbusdcBalance),
      label: 'BalanceManager quote balance for live DeepBook orders',
      ...(thresholdMetadata.DBUSDC || {}),
    },
    DEEP: {
      required: numericString(requiredDeepBalance),
      label: 'BalanceManager DEEP fee balance',
      ...(thresholdMetadata.DEEP || {}),
    },
    SUI_MIST: {
      required: numericString(requiredSuiGasMist),
      label: 'Agent wallet SUI gas balance',
      ...(thresholdMetadata.SUI_MIST || {}),
    },
  };
  const criteria = [
    {
  ...
  for (const row of criteria) {
    ...
    blockers.push({
      ...
      label: labels[row.blocker_code],
      holder: row.holder,
      asset: row.asset,
      observed: row.observed_balance,
