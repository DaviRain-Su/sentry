...
async function createPolicy({ marker, budgetCeiling, maxSlippageBps, expiresAtMs }) {
  ...
  assert(
    resolved.effects?.status?.status === 'success',
  ...
  assert(
    created?.wrapper_id && created?.mandate_id,
    'PolicyCreated event did not contain wrapper/mandate IDs',
    resolved.events
  );
  const event = (resolved.events || []).find((e) =>
    String(e.type).endsWith('::policy::PolicyCreated')
  );
  assert(
    hexFromMaybeVector(event?.parsedJson?.strategy_hash) === strategy.strategy_hash,
    'PolicyCreated strategy hash did not match current run'
  );
...
async function revokePolicy(policy) {
  const built = await postJson(`/api/policies/${policy.wrapper_id}/revoke`, {
    owner: ownerAddress,
    confirmed: true,
  });
  ...
  assert(
    event?.parsedJson?.wrapper_id === policy.wrapper_id,
    'PolicyRevoked event wrapper mismatch',
    event?.parsedJson
  );
  return resolved;
}
...
async function activitySummary(wrapperId) {
  ...
  return {
    ...
    execution_success_count: (detail.events || []).filter((e) => e.type === 'AgentTradeExecuted')
      .length,
...
async function assertBlockedPlan({ name, policy, proposed, expectedCode, mandateId }) {
  ...
  assert(
    response.code === expectedCode || response.blocker_code === expectedCode,
    `${name} returned wrong blocker`,
    response
  );
  ...
  assert(
    beforeWrapper?.spent_amount === afterWrapper?.spent_amount,
    `${name} changed wrapper spend`,
    { before: beforeWrapper, after: afterWrapper }
  );
  assert(beforeActivity.spent_amount === afterActivity.spent_amount, `${name} changed API spend`, {
    beforeActivity,
    afterActivity,
  });
  assert(
    beforeActivity.execution_success_count === afterActivity.execution_success_count,
    `${name} created execution success activity`,
    { beforeActivity, afterActivity }
  );
  assert(
    chainSuccessEvents.length === 0,
    `${name} created AgentTradeExecuted chain activity`,
    chainEvents
  );
...
const cases = [
  {
    ...
    proposed: {
      ...
      agent_id: delegatedAgentAddress,
    },
  },
  {
    name: 'over-slippage',
...
assert(
  Number(expiredMandate?.expires_at_ms) < expiredClockMs,
  'Expired policy setup did not pass Sui Clock expiry',
  { expiredClockMs, mandate: expiredMandate }
);
evidence.push(
  await assertBlockedPlan({
    name: 'expired-policy',
...
assert(
  revokedMandate?.revoked === true,
  'Active policy mandate was not revoked after revoke tx',
  revokedMandate
);
evidence.push(
  await assertBlockedPlan({
  ...
  JSON.stringify(
    {
      phase: 'pass',
      summary:
        'All Guardian/safety negative paths blocked before submission through Worker/API validate-plan evidence; wrapper spend and execution-success activity stayed unchanged.',
