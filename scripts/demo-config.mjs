...
console.log(`${line}\nRescueGrid — Sui Testnet deployment\n${line}`)
console.log(`chain                ${d.chain}`)
console.log(`rescuegrid package   ${d.rescuegrid.package_id}`)
...
console.log(`Deepbook pool        ${d.deepbook.pools[d.deepbook.default_pool].pool_id}  (${d.deepbook.default_pool})`)
...
console.log(`AGENT_KEY: set locally                 # generated: node worker/scripts/gen-agent-key.mjs`)
console.log(`INTERNAL_AGENT_TICK_TOKEN: set locally # never print the raw token in evidence`)
console.log(`RESCUEGRID_DEMO_MODE: true|false`)
console.log(`EXECUTION_ENABLED: false               # flip to true only once usable DBUSDC/DEEP funding is verified`)
