...
const POOL_BY_ASSET = { SUI: DB.pools.SUI_DBUSDC, DEEP: DB.pools.DEEP_DBUSDC, WAL: DB.pools.WAL_DBUSDC }
...
export function parseIntent(text, owner, defaults = {}, nowMs = Date.now()) {
  ...
  const guardian_warnings = [
    { code: 1, level: 'pass', label: 'Slippage bound', detail: `Capped at ${(max_slippage_bps / 100).toFixed(2)}% on-chain.` },
    { code: 2, level: 'pass', label: 'Budget ceiling', detail: `Policy hard-caps spend at ${budgetNum} USDC on-chain.` },
  ]
  ...
  return { status: 'ok', strategy, strategy_hash: strategyHash(strategy), agent_address: AGENT_ADDRESS, guardian_warnings, ptb_preview }
}
