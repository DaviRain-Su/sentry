          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <div
              style={{
                ...
                flexShrink: 0,
                background:
                  i < step ? 'var(--accent)' : i === step ? 'var(--accent-dim)' : 'var(--glass-2)',
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span
            className="mono"
            style={{ fontSize: 10, fontWeight: 700, color: m.c, letterSpacing: '0.08em' }}
          >
            {m.label}
          </span>
        </div>
...
function LegBuilder({ scenario, budget, leverage, legs, setLegs }) {
  ...
  const longWord = isSpot ? 'Buy' : 'Long',
    shortWord = isSpot ? 'Sell' : 'Short';
  ...
  const addLeg = () => {
    if (legs.length < 4)
      setLegs([
        ...legs,
        {
          venue: venues.find((v) => !legs.some((l) => l.venue === v)) || venues[0],
          side: 'short',
          pct: 50,
        },
      ]);
  ...
  const removeLeg = (i) => {
    if (legs.length > 1) setLegs(legs.filter((_, j) => j !== i));
  };
    ...
    <div>
      ...
      >
        ...
        <button
          ...
          style={{ padding: '5px 10px', opacity: legs.length >= 4 ? 0.5 : 1 }}
        >
          ...
          return (
            <div
              ...
              style={{
                ...
                borderRadius: 'var(--r-md)',
                background: 'var(--glass)',
                border: `1.5px solid ${sideC(l.side)}55`,
              }}
            >
              <div
                style={{
              ...
              >
                <span className="mono" style={{ fontSize: 9.5, color: 'var(--t2)' }}>
                  LEG {i + 1}
                </span>
                {legs.length > 1 && (
                  <button
                ...
                {[
                  ...
                  <button
                    ...
                    style={{
                      ...
                      background:
                        l.side === s
                          ? `color-mix(in srgb, ${sideC(s)} 18%, transparent)`
                          : 'transparent',
                      color: l.side === s ? sideC(s) : 'var(--t2)',
                    }}
                  >
                    {lbl}
                  </button>
              ...
              <select
                value={l.venue}
                onChange={(e) => setLeg(i, { venue: e.target.value })}
                style={{
                  width: '100%',
                  padding: '7px 9px',
                  borderRadius: 'var(--r-sm)',
              ...
              <input
                ...
                step="5"
                value={l.pct}
                onChange={(e) => setLeg(i, { pct: +e.target.value })}
                className="rg-slider"
              />
              {liqPx && (
      ...
      >
        ...
        >
          ...
          <span
            style={{
              fontSize: 11,
              fontWeight: 700,
              color: neutral ? 'var(--safe)' : 'var(--warn)',
            }}
          >
            {neutral
              ? 'market-neutral'
              : (net > 0 ? 'net long ' : 'net short ') + Math.abs(net) + '%'}
          </span>
        </div>
        <div
        ...
        >
          ...
          <div
            style={{
              ...
              height: 14,
              borderRadius: '50%',
              background: neutral ? 'var(--safe)' : 'var(--warn)',
              boxShadow: `0 0 8px ${neutral ? 'var(--safe)' : 'var(--warn)'}`,
              transition: 'left .2s',
            }}
          />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 7 }}>
          <span className="mono" style={{ fontSize: 9.5, color: 'var(--t3)' }}>
            {shortWord} ${fmtUsd((budget * sumShort) / 100, 0)}
          </span>
          <span className="mono" style={{ fontSize: 9.5, color: 'var(--t3)' }}>
            {longWord} ${fmtUsd((budget * sumLong) / 100, 0)}
          </span>
        </div>
        {!neutral && (
          <div
            style={{ fontSize: 10.5, color: 'var(--warn)', marginTop: 8, display: 'flex', gap: 6 }}
          >
            <Icon name="alert" size={12} style={{ flexShrink: 0, marginTop: 1 }} />
            Legs are unbalanced — this strategy carries directional risk. Match{' '}
            {longWord.toLowerCase()} and {shortWord.toLowerCase()} sizes for neutrality.
          </div>
        )}
      </div>
    </div>
...
export function NewStrategy({ onDone, mode, setMode, seed }) {
  const PARSED = {
    safe: RG.parsed,
    dca: RG.parsedDCA,
    hedge: RG.parsedHedge,
    risky: RG.parsedRisky,
    'funding-arb': RG.parsedFundingArb,
    lp: RG.parsedLP,
    lend: RG.parsedLendYield,
    spot: RG.parsedSpotArb,
  };
  ...
  const classify = (s) => {
    ...
    if (/stablecoin|idle|yield router|lend|lending|supply|money market|best (rate|yield)/i.test(s))
      return 'lend';
      ...
      if (live) {
        ...
        try {
          ...
          if (preview?.status === 'ok' && preview.strategy) {
            ...
            const bud = preview.strategy.budget_ceiling
              ? Number(preview.strategy.budget_ceiling) / 1e6
              : values.budget;
            try {
              const h = await getSuiPriceHistory(30);
              backtest =
                h.status === 'ok' && h.prices.length > 2
                  ? runBacktest(h.prices, { thresholdPct: th, budget: bud })
                  : null;
            } catch {
              backtest = null;
            }
  ...
  return (
    <div
      style={{ maxWidth: 880, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}
    >
      ...
      <form.Subscribe selector={(state) => state.values}>
        {(values) => {
          ...
          const setRequireApproval = (next) =>
            form.setFieldValue(
              'requireApproval',
              typeof next === 'function' ? next(requireApproval) : next
            );
          const setLegs = (next) =>
            form.setFieldValue('legs', typeof next === 'function' ? next(legs) : next);
              ...
              {step === 0 && (
                <div className="card fade-up" style={{ padding: 24 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 14 }}>
                    ...
                    <Button
                      size="sm"
                      onPress={() => setText(RG.riskyExample)}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 22 }}>
                    <Button
                      ...
                      isDisabled={!text.trim() || parsing}
                      onPress={() => parse(values)}
                    >
                      {parsing ? (
                        <>
                          <Icon
                        ...
                        <>
                          Parse intent <Icon name="chevR" size={15} />
                        </>
                      )}
                    </Button>
                  </div>
              ...
              {step === 1 && (
                ...
                >
                    ...
                    <div className="card" style={{ padding: 18, borderColor: 'var(--accent)' }}>
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          ...
                          <div className="card-title">
                            {livePreviewSource === 'Worker'
                              ? 'On-chain parse · Worker'
                              : 'Local preview'}
                          </div>
                        </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                        {livePreview.ptb_preview.map((l, i) => (
                          <div key={i} style={{ fontSize: 12, color: 'var(--t1)' }}>
                      ...
                      {livePreview.guardian_warnings?.length > 0 && (
                        <div style={{ marginTop: 12, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                          {livePreview.guardian_warnings.map((g, i) => (
                            <span
                              key={i}
                              className={`badge badge-${g.level === 'fail' ? 'danger' : g.level === 'warn' ? 'warn' : 'safe'}`}
                              style={{ fontSize: 10 }}
                            >
                  ...
                  {livePreview?.status === 'error' && (
                    <div
                      className="card"
                      style={{ padding: 14, borderColor: 'rgba(255,84,112,0.4)' }}
                    >
                      <span className="badge badge-danger" style={{ fontSize: 10 }}>
                        worker parse: {livePreview.code}
                      </span>
                  ...
                  <div className="card" style={{ padding: 22 }}>
                    <div className="badge badge-accent" style={{ marginBottom: 12 }}>
                      <Icon name="sparkles" size={12} /> Interpreted intent
                    </div>
                    <h2 className="display" style={{ fontSize: 18, fontWeight: 600 }}>
                      {P.intent}
                    </h2>
                    <p
                      style={{ color: 'var(--t1)', fontSize: 13.5, marginTop: 6, lineHeight: 1.55 }}
                    >
                      {P.summary}
                    </p>
                    <div
                    ...
                    >
                      {P.params.map((pp) => (
                        ...
                        >
                          ...
                          <div
                            className="mono"
                            style={{ fontSize: 13, fontWeight: 600, marginTop: 5 }}
                          >
                            {pp.v}
                          </div>
                        </div>
                  ...
                  {BT && (
                    <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                      <div className="card-hd" style={{ paddingBottom: 14 }}>
                        ...
                        <span
                          className={`badge ${btLive ? 'badge-accent' : 'badge-neutral'}`}
                          style={{ fontSize: 9.5 }}
                        >
                          {btLive ? 'real · DeepBook mainnet' : 'simulated · last 30d'}
                        </span>
                      </div>
                      <div
                        style={{
                        ...
                        <div>
                              ...
                              <div key={s.k}>
                                <div
                                  ...
                                  style={{
                                    fontSize: 18,
                                    fontWeight: 600,
                                    color: s.v.startsWith('+') ? 'var(--safe)' : 'var(--t0)',
                                  }}
                                >
                                  {s.v}
                                </div>
                                <div style={{ fontSize: 10.5, color: 'var(--t2)', marginTop: 2 }}>
                                  {s.k}
                                </div>
                              </div>
                            ))}
                          ...
                          {btLive && (
                            <div style={{ fontSize: 10.5, color: 'var(--t3)', marginTop: 8 }}>
                              Simulated on real SUI/USDC daily prices from DeepBook mainnet using
                              your parsed threshold + budget. Past performance is not indicative of
                              future results.
                            </div>
                          )}
                        </div>
                  ...
                  <div className="rg-2col" style={{ alignItems: 'start' }}>
                    ...
                    <div className="card">
                      <div className="card-hd" style={{ paddingBottom: 12 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ color: blocked ? 'var(--danger)' : 'var(--accent)' }}>
                        ...
                        {blocked ? (
                          ...
                          <span className="badge badge-warn">
                            <span className="dot"></span>1 to review
                          </span>
                        )}
                      </div>
                      <div
                  ...
                  {blocked && (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          Guardian blocked this strategy
                        </div>
                        <div style={{ fontSize: 12.5, color: 'var(--t1)', marginTop: 1 }}>
                          {failCount} critical risks must be resolved before a Policy Object can be
                          created. The agent will never deploy an unsafe intent.
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <Button
                      className="rg-btn-2"
                      onPress={() => setStep(0)}
                      startContent={<Icon name="chevL" size={15} />}
                    >
                      Edit intent
                    </Button>
                    {blocked ? (
                      <Button isDisabled className="opacity-50">
                        <Icon name="x" size={15} /> Blocked by Guardian
                      </Button>
                    ) : (
                      <Button
                        className="bg-accent text-accent-foreground font-semibold"
              ...
              {step === 2 && (
                <div className="fade-up card" style={{ padding: 24 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                    ...
                    <div>
                      <label
                        style={{ fontSize: 13, fontWeight: 600, display: 'block', marginBottom: 8 }}
                      >
                            ...
                            <div>
                              ...
                              >
                                ...
                                <span
                                  ...
                                  style={{
                                    fontSize: 14,
                                    color: leverage > 4 ? 'var(--warn)' : 'var(--accent)',
                                    fontWeight: 600,
                                  }}
                                >
                                  {leverage.toFixed(1)}×
                              ...
                              />
                              <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 6 }}>
                                Caps notional per leg. Higher leverage tightens the liquidation
                                buffer.
                              </div>
                            </div>
                          )}
                            ...
                            <div>
                              ...
                              >
                                ...
                                <span
                                  ...
                                  style={{
                                    fontSize: 14,
                                    color: liqBuffer < 15 ? 'var(--danger)' : 'var(--accent)',
                                    fontWeight: 600,
                                  }}
                                >
                                  {liqBuffer}%
                              ...
                              <input
                                ...
                                step="1"
                                value={liqBuffer}
                                onChange={(e) => setLiqBuffer(+e.target.value)}
                                className="rg-slider"
                              />
                              <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 6 }}>
                                Agent deleverages a leg before its margin gets this close to
                                liquidation.
                              </div>
                            </div>
                          )}
                            ...
                            <div>
                              ...
                              >
                                ...
                                <span
                                  ...
                                  style={{
                                    fontSize: 14,
                                    color: liqBuffer > 70 ? 'var(--warn)' : 'var(--accent)',
                                    fontWeight: 600,
                                  }}
                                >
                                  {liqBuffer}%
                              ...
                              />
                              <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 6 }}>
                                Loan-to-value ceiling if the optimizer ever borrows to loop.
                                Supply-only by default.
                              </div>
                            </div>
                          )}
                          {adv.flip && (
                            <div>
                              ...
                              <input
                                ...
                                step="0.5"
                                value={flipThresh}
                                onChange={(e) => setFlipThresh(+e.target.value)}
                                className="rg-slider"
                              />
                              <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 6 }}>
                                Funding-flip guard — closes both legs if the spread decays past this
                                floor.
                              </div>
                            </div>
                          )}
                    ...
                    <div
                      ...
                      style={{
                        ...
                        padding: '14px 16px',
                        borderRadius: 'var(--r-md)',
                        border: `1.5px solid ${requireApproval ? 'var(--accent)' : 'var(--border)'}`,
                        background: requireApproval ? 'var(--accent-dim)' : 'var(--glass)',
                        transition: 'all .15s',
                      }}
                    >
                      <div
                        style={{
                          ...
                          flexShrink: 0,
                          padding: 3,
                          background: requireApproval ? 'var(--accent)' : 'var(--bg-0)',
                      ...
                      >
                        <div
                          style={{
                            ...
                            height: 18,
                            borderRadius: '50%',
                            background: requireApproval ? '#06231f' : 'var(--t2)',
                            transform: requireApproval ? 'translateX(16px)' : 'none',
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 2 }}>
                          {requireApproval
                            ? 'Agent stages every order and waits for your sign-off — fully supervised.'
                            : 'Agent executes autonomously within these limits — set-and-forget.'}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 26 }}>
                    ...
                    <Button
                      ...
                      onPress={() => setStep(3)}
                    >
                      Choose run mode <Icon name="chevR" size={15} />
                    </Button>
                  </div>
                </div>
              )}
                      ...
                      {[
                        ...
                        <div
                          ...
                          style={{
                            ...
                            padding: 18,
                            borderRadius: 'var(--r-lg)',
                            background: mode === m.id ? 'var(--accent-dim)' : 'var(--glass)',
                        ...
                        >
                          ...
                          >
                            <div
                              style={{
                                ...
                                height: 38,
                                borderRadius: 10,
                                background: mode === m.id ? 'var(--accent)' : 'var(--glass-hi)',
                                color: mode === m.id ? '#06231f' : 'var(--t1)',
                                display: 'flex',
                                alignItems: 'center',
                                justifyContent: 'center',
                              }}
                            ...
                            <div
                              style={{
                                ...
                                height: 20,
                                borderRadius: '50%',
                                border: `2px solid ${mode === m.id ? 'var(--accent)' : 'var(--border-hi)'}`,
                                display: 'flex',
                                alignItems: 'center',
                                justifyContent: 'center',
                              }}
                          ...
                          <div
                            className="display"
                            style={{ fontWeight: 600, fontSize: 16, marginTop: 14 }}
                          >
                            {m.t}
                          </div>
                          <p
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div
                        style={{
                          ...
                          height: 38,
                          borderRadius: 10,
                          background: requireApproval ? 'var(--warn-dim)' : 'var(--sui-dim)',
                          color: requireApproval ? 'var(--warn)' : 'var(--sui)',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                        }}
                        ...
                        onDone(
                          {
                            ...
                            slip,
                            expiry,
                            leverage: adv.leverage ? leverage : null,
                            liqBuffer: adv.leverage || adv.ltv ? liqBuffer : null,
                            requireApproval,
