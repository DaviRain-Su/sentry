...
function StrategyCard({ s, onDeploy, onToast, onOpen }) {
  ...
  const action = (e) => {
    ...
    if (s.status === 'soon') {
      onToast && onToast('Coming soon — we’ll flag it the moment the adapter ships', 'var(--sui)');
      return;
    }
    if (s.watch) {
      onToast &&
        onToast('Watchtower armed — monitoring only, zero execution authority', 'var(--safe)');
      return;
    }
    if (s.scenario) {
      onDeploy && onDeploy({ scenario: s.scenario });
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        <div
          style={{
            ...
            background: `color-mix(in srgb, ${cc} 16%, transparent)`,
            color: cc,
          }}
        >
      ...
      {/* blurb */}
      <div style={{ fontSize: 12.5, color: 'var(--t1)', lineHeight: 1.5, minHeight: 56 }}>
        {s.blurb}
      </div>
        ...
        <div>
          ...
          <div
            ...
            style={{
              fontSize: 18,
              fontWeight: 600,
              color: deployable ? 'var(--accent)' : 'var(--t0)',
              marginTop: 2,
            }}
          >
            {s.metric.v}
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        {s.adapters.map((a) => (
          ...
          >
            <span
              style={{ width: 6, height: 6, borderRadius: '50%', background: adapterColor(a) }}
            />
            {a}
          </span>
        ))}
        ...
        {s.risks.length === 0 ? (
          ...
          s.risks.map((r) => {
            ...
            return (
              ...
              >
                <span style={{ width: 6, height: 6, borderRadius: 2, background: rt.c }} />
                {rt.label}
              </span>
            );
          })
      ...
      <button
        onClick={action}
        className={`btn btn-sm ${deployable && !s.watch ? 'btn-primary' : ''}`}
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ flex: 1, minWidth: 240 }}>
          <div className="display" style={{ fontSize: 16, fontWeight: 600 }}>
            Policy-constrained strategy templates
          </div>
          <div
            style={{
              fontSize: 12.5,
          ...
          >
            Every template deploys as a Move Policy Object with its own Guardian rules and venue
            scope — the agent can act, but never beyond what you authorize.
          </div>
        </div>
        <div style={{ display: 'flex', gap: 22 }}>
          {[
            ['available', counts.available, 'var(--safe)'],
            ['testnet', counts.testnet, 'var(--warn)'],
            ['coming soon', counts.soon, 'var(--t2)'],
          ].map(([k, v, c]) => (
            <div key={k}>
              <div className="mono display" style={{ fontSize: 22, fontWeight: 600, color: c }}>
                {v}
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
        {list.map((s) => (
          <StrategyCard key={s.id} s={s} onDeploy={onDeploy} onToast={onToast} onOpen={onOpen} />
        ))}
      </div>
    </div>
  );
...
const DETAIL_PARSED = {
  ...
  dca: 'parsedDCA',
  hedge: 'parsedHedge',
  'funding-arb': 'parsedFundingArb',
  lp: 'parsedLP',
  lend: 'parsedLendYield',
  spot: 'parsedSpotArb',
};
...
export function StrategyDetail({ id, onBack, onDeploy, onToast }) {
  ...
  const riskParts =
    ...
    s.risks.map((k) => ({
      ...
      level: 'warn',
      note:
        (RG.riskTax[k] ? RG.riskTax[k].label : k) +
        ' exposure applies — sized and capped by policy.',
    }));
  const permissions = d.permissions || [
    'Act only on scoped venues',
    'Read price + rate feeds',
    'Never withdraw or transfer funds off-app',
  ];
  const timeline = d.timeline || [
    { t: 'trigger', d: 'Condition met' },
    { t: 't0', d: 'Agent executes within policy' },
    { t: 'ongoing', d: 'Monitor, rebalance, log on-chain' },
  ];
  ...
  const sideC = (side) =>
    /short|sell/i.test(side)
      ...
      : /long|buy|supply|lp/i.test(side)
        ...
        : 'var(--t1)';
  const GLV = {
    pass: ['var(--safe)', 'var(--safe-dim)', 'check'],
    warn: ['var(--warn)', 'var(--warn-dim)', 'alert'],
    fail: ['var(--danger)', 'var(--danger-dim)', 'x'],
  };
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 240 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
            ...
            <span className={`badge ${st.cls}`} style={{ fontSize: 9.5 }}>
              <span className="dot"></span>
              {st.label}
            </span>
          </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            ...
            onClick={() =>
              onToast &&
              onToast(
                'Dry-run scheduled — simulating against your wallet & live books',
                'var(--sui)'
              )
            }
          >
          ...
          {deployable ? (
            <button
              ...
              onClick={() => onDeploy && onDeploy({ scenario: s.scenario })}
            >
              <Icon name="bolt" size={14} /> Preview policy
            </button>
          ) : (
            <button className="btn btn-sm" disabled style={{ opacity: 0.5 }}>
              {s.status === 'soon' ? 'Coming soon' : 'Watch-only'}
            </button>
          )}
        </div>
      </div>
                ...
                {legs.map((l, i) => (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${sideC(l.side)}55`,
                    }}
                  >
                    <span
                      style={{ fontSize: 10, fontWeight: 700, color: sideC(l.side), minWidth: 64 }}
                    >
                      {l.side.toUpperCase()}
                    </span>
                    <div>
          ...
          <Card title="Position legs" icon="layers" pad={0}>
            <div style={{ padding: '0 0 4px' }}>
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: '1.2fr 1.2fr 0.8fr 1fr 1.4fr',
                  padding: '0 18px 10px',
                }}
              >
              ...
              {legs.map((l, i) => (
                ...
                >
                  ...
                  <div
                    className="mono"
                    style={{ fontSize: 11.5, textAlign: 'right', color: l.expC || 'var(--t1)' }}
                  >
                    {l.exp}
                  </div>
                </div>
          ...
          <Card title="Execution timeline" icon="clock">
            ...
            >
              {timeline.map((tl, i) => (
                <div
                  key={i}
                  style={{ position: 'relative', paddingBottom: i < timeline.length - 1 ? 16 : 0 }}
                >
                  <div
                    style={{
                      position: 'absolute',
                  ...
                  />
                  <div className="mono" style={{ fontSize: 10.5, color: cc, fontWeight: 600 }}>
                    {tl.t}
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <Card title="Yield decomposition" icon="percent">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              {yieldParts.map((y, i) => (
                ...
                >
                  ...
                  <span className="mono" style={{ fontWeight: 600, color: y.c }}>
                    {y.v}
                  </span>
                </div>
              ))}
            ...
            <div style={{ fontSize: 10.5, color: 'var(--t2)', marginTop: 12, lineHeight: 1.45 }}>
              Every number is annualized and net of the cost lines above — no single headline APY.
            </div>
          </Card>
          ...
          <Card title="Risk decomposition" icon="shield">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
              {riskParts.length === 0 && (
                <div style={{ fontSize: 12.5, color: 'var(--safe)' }}>
                  No execution authority — monitoring only.
                </div>
              )}
              {riskParts.map((r, i) => {
                    ...
                    <div key={x.k}>
                      <div
                        ...
                        style={{
                          fontSize: 15,
                          fontWeight: 600,
                          color: x.v.startsWith('+') ? 'var(--safe)' : 'var(--t0)',
                        }}
                      >
                        {x.v}
                      </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              {permissions.map((p, i) => (
                ...
                >
                  <span
                    style={{
                      color: /never|no /i.test(p) ? 'var(--danger)' : 'var(--safe)',
                      flexShrink: 0,
                      marginTop: 1,
                    }}
                  >
                    <Icon name={/never|no /i.test(p) ? 'x' : 'check'} size={13} stroke={2.4} />
                  </span>
                  {p}
                </div>
              ))}
                ...
                {guardian.map((g, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <Icon name={lv[2]} size={13} stroke={2.2} />
                      </span>
                      <div>
