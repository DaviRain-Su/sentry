...
export function ActivityView({ activity, onTx, live = false, loading = false, source = null }) {
  ...
  const sourceTone =
    sourceMeta.tone === 'warn' ? 'warn' : sourceMeta.kind === 'demo' ? 'demo' : 'live';
  const sourceBg =
    sourceTone === 'warn'
      ? 'var(--warn-dim)'
      : sourceTone === 'live'
  ...
  const sourceBorder =
    sourceTone === 'warn'
      ? 'rgba(255,194,75,0.35)'
      : sourceTone === 'live'
        ? 'color-mix(in srgb, var(--accent) 25%, var(--border))'
        : 'var(--border)';
  const sourceColor =
    sourceTone === 'warn' ? 'var(--warn)' : sourceTone === 'live' ? 'var(--accent)' : 'var(--t1)';
  const filtered = activity.filter((a) =>
    filter === 'all'
      ? true
      : ['executed', 'blocked', 'planned'].includes(filter)
        ? outcomeOf(a) === filter
        : filter === 'retry'
          ? a.kind === 'retry' || a.kind === 'fail'
          : a.kind === filter
  );
  ...
  const expand = (a) => {
    ...
    const reason =
      {
        ...
        rebalance:
          'Net exposure drifted past the policy threshold; the agent restored the target position.',
        bridge:
          'A leg needed collateral on another chain; the agent bridged funds to keep both sides funded.',
        guardian:
          'A pre-execution Guardian check failed, so the agent declined to act and logged the reason.',
        fail: 'The order was submitted but the book moved past the limit before settlement; no funds were spent.',
        retry:
          'A prior attempt failed; the agent re-quoted and resubmitted within the same limits.',
        monitor:
          'A scheduled risk evaluation ran. Conditions stayed inside policy, so no action was taken.',
        policy: liveSource
    ...
    const plan = {
      exec: [
        'assert_within_budget(cap, spent)',
        'deepbook::place_limit_order(...)',
        'log_activity(action="exec")',
      ],
      rebalance: ['read_net_delta()', 'adjust_legs(target≈0)', 'log_activity(action="rebalance")'],
      bridge: [
        'assert_within_budget(cap, spent)',
        'debridge::send(to, asset, amt)',
        'await_settlement()',
      ],
      guardian: [
        'assert_pool_liquidity(min)',
        '→ FAILED · abort()',
        'log_activity(action="blocked")',
      ],
  ...
  return (
    <div
      style={{ maxWidth: 920, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 18 }}
    >
      <div
        style={{
          display: 'flex',
      ...
      >
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          {kinds.map((k) => (
            <Fragment key={k.id}>
              ...
              <Button
                onPress={() => setFilter(k.id)}
                size="sm"
                className={
                  filter === k.id ? 'bg-accent text-accent-foreground font-semibold' : 'rg-btn-2'
                }
              >
                {k.label}
              </Button>
        ...
        <div className={`badge ${sourceMeta.badgeClass || 'badge-neutral'}`}>
          <Icon name={sourceMeta.icon || 'eye'} size={12} /> {sourceMeta.label}
        </div>
      </div>
      ...
      >
        <span style={{ color: sourceColor, flexShrink: 0 }}>
          <Icon name={sourceMeta.icon || 'eye'} size={15} />
        </span>
      ...
      {!loading && activity.length === 0 && (
        <EmptyCard
          ...
          detail={
            liveSource
              ? 'Autonomous executions, policy creations and revocations will appear here once your agent acts in the live source.'
              : 'No demo activity rows are available.'
          }
        />
      )}
      ...
      {!loading &&
        dates.map((date) => (
          <div key={date}>
            <div className="eyebrow" style={{ marginBottom: 10, marginLeft: 2 }}>
              {date}
            </div>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          >
                            ...
                            <span
                              ...
                              style={{
                                fontSize: 9,
                                background: `color-mix(in srgb, ${oc.c} 14%, transparent)`,
                                color: oc.c,
                              }}
                            >
                      ...
                      {isOpen && (
                        ...
                        >
                          ...
                          >
                            <span
                              className="eyebrow"
                              style={{ fontSize: 8.5, display: 'block', marginBottom: 5 }}
                            >
                          ...
                          <div>
                            <span
                              className="eyebrow"
                              style={{ fontSize: 8.5, display: 'block', marginBottom: 8 }}
                            >
                              Input snapshot
                            </span>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                              {[
                                [
                                  'Price',
                                  '$' +
                                    (ex.px.usd < 1 ? ex.px.usd.toFixed(4) : ex.px.usd.toFixed(3)),
                                ],
                                ['24h', ex.px.chg + '%'],
                                ['Risk score', a.risk != null ? a.risk : '—'],
                                ['Mode', a.mode || '—'],
                              ].map(([k, v]) => (
                                <div
                                  key={k}
                                ...
                                >
                                  ...
                                  <span className="mono" style={{ color: 'var(--t0)' }}>
                                    {v}
                                  </span>
                                </div>
                              ))}
                          ...
                          <div>
                            <span
                              ...
                              style={{ fontSize: 8.5, display: 'block', marginBottom: 8 }}
                            >
                              {liveSource ? 'Execution plan (PTB)' : 'Execution plan preview'}
                            </span>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                              {ex.plan.map((stepp, j) => (
                                <div
                                  ...
                                  style={{
                                    fontSize: 10.5,
                                    color: /FAILED|moved|abort|revert/.test(stepp)
                                      ? 'var(--danger)'
                                      : /no-op|within policy/.test(stepp)
                                        ? 'var(--t2)'
                                        : 'var(--t1)',
                                    padding: '5px 9px',
                                    borderRadius: 6,
                                ...
                                >
                                  <span style={{ color: 'var(--t3)' }}>{j + 1}</span>
                                  {stepp}
                                </div>
                              ))}
                            </div>
                          ...
                          >
                            <div
                              style={{
                                ...
                                borderRadius: 'var(--r-sm)',
                                background:
                                  ex.oc === 'blocked' ? 'var(--danger-dim)' : 'var(--safe-dim)',
                                border: `1px solid color-mix(in srgb, ${oc.c} 30%, var(--border))`,
                              }}
                            >
                              <span style={{ color: oc.c, flexShrink: 0 }}>
                                <Icon
                                  name={ex.oc === 'blocked' ? 'shield' : 'check'}
                                  size={15}
                                  stroke={2.2}
                                />
                              </span>
                              <div>
                                ...
                                <div style={{ fontSize: 10.5, color: 'var(--t2)' }}>
                                  {ex.oc === 'blocked'
                                    ? 'Order rejected before any funds moved'
                                    : liveSource
                                      ? 'Budget, slippage and scope verified on-chain'
                                      : 'Budget, slippage and scope shown as a simulated policy check'}
                                </div>
                              </div>
                            </div>
                            <div
                            ...
                            >
                              ...
                              <div>
                                ...
                                <div className="mono" style={{ fontSize: 13, fontWeight: 600 }}>
                                  {a.amount < 0 ? '−$' + fmtUsd(Math.abs(a.amount)) : '$0'}
                                </div>
                              </div>
                            </div>
                          </div>
    ...
    <div className="card" style={{ padding: 20, opacity: active ? 1 : 0.72 }}>
      ...
      >
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
              ...
              <span className="badge badge-neutral" style={{ fontSize: 9.5, padding: '2px 7px' }}>
                <Icon name={p.mode === 'cloud' ? 'cloud' : 'cpu'} size={10} />
                {p.mode}
              </span>
            </div>
          </div>
        ...
        <span className={`badge ${statusMeta.cls}`}>
          <span className={`dot ${statusMeta.pulse ? 'pulse' : ''}`}></span>
          {statusMeta.label}
        </span>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        >
          ...
          <span className="mono">
            <span style={{ color: 'var(--t0)', fontWeight: 600 }}>{p.budgetUsed}</span>{' '}
            <span style={{ color: 'var(--t2)' }}>/ {p.budgetCap} USDC</span>
          </span>
        </div>
        <ProgressBar
      ...
      >
        {[
          ...
          >
            ...
            <div className="mono" style={{ fontSize: 12.5, fontWeight: 600, marginTop: 4 }}>
              {x.v}
            </div>
          </div>
        ))}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span
          className="mono"
          style={{ fontSize: 11.5, color: days < 5 ? 'var(--warn)' : 'var(--t2)' }}
        >
          <Icon name="clock" size={12} style={{ verticalAlign: -2, marginRight: 4 }} />
          expires in {days}d
        </span>
        <div style={{ display: 'flex', gap: 8 }}>
          {onLive && p.status !== 'revoked' && (
            <Button
              size="sm"
              className="rg-btn-2"
              onPress={() => onLive(p)}
              startContent={<Icon name="activity" size={13} />}
            >
              Live
            </Button>
          )}
          <Button
            ...
            className="rg-btn-2"
            onPress={() => onInspect(p)}
            startContent={<Icon name="eye" size={13} />}
          >
            Inspect
          </Button>
          <Button
            size="sm"
            isDisabled={readOnly || p.status === 'revoked'}
            onPress={() => onRevoke(p.id)}
            className="bg-danger text-white"
          >
            <Icon name="x" size={13} stroke={2.4} />{' '}
            {readOnly ? 'Read-only' : p.status === 'revoked' ? 'Revoked' : 'Revoke'}
          </Button>
        </div>
      </div>
    </div>
  ...
  const policySourceCopy =
    {
      ...
      loading:
        'Loading live policies from Worker or chain. Demo policy rows are not mixed into this list.',
      error:
        'Live policy reads failed. Demo policy rows are not used as a fallback in this live view.',
      fallback:
        'Worker reads failed, so these values come from direct chain read-only fallback where available.',
      worker: readOnly
    ...
    >
      ...
      >
        <div>
          ...
          <div className="mono display" style={{ fontSize: 24, fontWeight: 600, marginTop: 4 }}>
        ...
        <div>
          ...
          <div
            ...
            style={{ fontSize: 24, fontWeight: 600, marginTop: 4, color: 'var(--accent)' }}
          >
            ${fmtUsd(totalUsed, 0)}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 280, fontSize: 12, color: 'var(--t2)', lineHeight: 1.5 }}>
          <span
            className={`badge ${sourceMeta.badgeClass || 'badge-neutral'}`}
            style={{ fontSize: 9.5, marginBottom: 7 }}
          >
            <Icon name={sourceMeta.icon || 'eye'} size={10} />
            {sourceMeta.label}
          </span>
      ...
      {!loading && policies.length === 0 && (
        <EmptyCard
          ...
          detail={
            liveSource
              ? 'Create a strategy to mint your first Move Policy Object. The agent gets no authority until that transaction succeeds.'
              : 'Create a strategy to preview the policy shape in demo mode.'
          }
        />
      )}
      {!loading && policies.length > 0 && (
