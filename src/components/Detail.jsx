    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0' }}>
      <div
        style={{
          ...
          background: granted ? 'var(--safe-dim)' : 'var(--danger-dim)',
          color: granted ? 'var(--safe)' : 'var(--danger)',
        }}
      >
        <Icon name={granted ? 'check' : 'x'} size={13} stroke={2.6} />
      </div>
      <div style={{ flex: 1 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: granted ? 'var(--t0)' : 'var(--t1)' }}>
          {label}
        </span>
        <span className="mono" style={{ fontSize: 11, color: 'var(--t2)', marginLeft: 8 }}>
          {fn}
        </span>
      </div>
      <span
...
export function PolicyInspect({
  ...
  const auditLabel = liveSource
    ? `Audit trail · ${log.length} on-chain events`
    : `Demo audit trail · ${log.length} simulated events`;
  ...
  const protocols = [
    {
      ...
      kind: 'CLOB · spot',
      on: p.scope.length > 0,
      note: 'order book — place / cancel limit orders',
    },
  ...
  const structLines = [
    ...
    { indent: 1, key: 'id', val: 'UID' },
    {
      indent: 1,
      key: 'owner',
      val: `address  // ${p.owner ? p.owner.slice(0, 10) + '…' + p.owner.slice(-6) : RG.user.addr}`,
    },
  ...
  return (
    ...
    >
      ...
      >
          ...
          >
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                <span className="mono" style={{ fontSize: 11.5, color: 'var(--sui)' }}>
                  {p.id}
                </span>
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 22 }}>
          ...
          <div>
            ...
            >
              ...
              <span className="mono">
                <span style={{ color: 'var(--accent)', fontWeight: 700 }}>{p.budgetUsed}</span>
                <span style={{ color: 'var(--t2)' }}>
                  {' '}
                  / {p.budgetCap} USDC · {pct}%
                </span>
              </span>
            </div>
            <div
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: 100,
                  background:
                    pct > 80 ? 'var(--danger)' : 'linear-gradient(90deg,var(--accent),#1fc7b1)',
            ...
            <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 7 }}>
              {liveSource ? (
                <>
                  ...
                  before every order. Exceeding the cap aborts the transaction on-chain — it is
            ...
            >
              {structLines.map((l, i) => (
                <div key={i} style={{ paddingLeft: (l.indent || 0) * 18, whiteSpace: 'pre' }}>
                  ...
                  {l.key && (
                    <>
                      <span style={{ color: 'var(--t0)' }}>{l.key}</span>
                      <span style={{ color: 'var(--t2)' }}>: </span>
                      <span style={{ color: 'var(--t2)' }}>{l.val}</span>
          ...
          <div>
            ...
            >
              ...
              <span style={{ fontSize: 11, color: 'var(--t2)' }}>
                scope is extensible per-policy
              </span>
            </div>
              ...
              {protocols.map((pr) => (
                <div
                  ...
                  style={{
                    ...
                    background: pr.on ? 'var(--accent-dim)' : 'var(--glass)',
                    border: `1px solid ${pr.on ? 'var(--accent)' : 'var(--border)'}`,
                    opacity: pr.on ? 1 : 0.55,
                  }}
                >
                  <div
                    style={{
                      ...
                      height: 8,
                      borderRadius: '50%',
                      background: pr.on ? 'var(--accent)' : 'var(--t3)',
                      flexShrink: 0,
                      boxShadow: pr.on ? '0 0 8px var(--accent-glow)' : 'none',
                    }}
                  />
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              >
                <span style={{ color: 'var(--sui)', flexShrink: 0 }}>
                  <Icon name="wallet" size={16} />
                </span>
                <div>
                  <div style={{ fontSize: 12.5, fontWeight: 600 }}>
                    {liveSource ? 'Signs with a session key' : 'Session key model'}
                  </div>
                  <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 2 }}>
                    {liveSource ? (
                      <>
                        The agent uses the delegated{' '}
                        <span className="mono" style={{ color: 'var(--t1)' }}>
                          AgentCap
                        </span>{' '}
                        — never your zkLogin key. Revoke it and signing power vanishes.
                      </>
                    ) : (
                      <>
                        Demo mode previews the delegated{' '}
                        <span className="mono" style={{ color: 'var(--t1)' }}>
                          AgentCap
                        </span>{' '}
                        model. No session key is active until a real policy is minted.
                      </>
                    )}
                  </div>
                </div>
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 12.5, fontWeight: 600 }}>
                    {liveSource ? 'Gas is sponsored' : 'Sponsored gas model'}
                  </div>
                  <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 2 }}>
                    {liveSource
                      ? 'Fees are paid by a gas station via sponsored transactions, so the agent needs no SUI of its own to act autonomously.'
                      : 'Demo mode shows the intended sponsored transaction path. No gas is spent by these sample rows.'}
                  </div>
                </div>
              </div>
              <div
              ...
              >
                ...
                <div>
                  <div style={{ fontSize: 12.5, fontWeight: 600 }}>
                    {liveSource ? 'Triggers read a Pyth feed' : 'Trigger model'}
                  </div>
                  <div style={{ fontSize: 11.5, color: 'var(--t2)', marginTop: 2 }}>
                    {liveSource ? (
                      <>
                        Price conditions are asserted on-chain against the{' '}
                        <span className="mono" style={{ color: 'var(--t1)' }}>
                          Pyth
                        </span>{' '}
                        oracle — the agent can't fake a trigger.
                      </>
                    ) : (
                      <>
                        Demo mode previews Pyth-backed trigger checks. Real assertions happen
                        on-chain only after minting a policy.
                      </>
                    )}
                  </div>
                </div>
            ...
            >
              {log.length === 0 && (
                <div style={{ fontSize: 12.5, color: 'var(--t2)' }}>
                  {liveSource
                    ? 'No on-chain events yet — the agent is monitoring.'
                    : 'No simulated events yet in this demo policy.'}
                </div>
              )}
              {log.map((a, i) => (
                <div
                  key={i}
                  style={{ position: 'relative', paddingBottom: i < log.length - 1 ? 16 : 0 }}
                >
                  <div
                    style={{
                      position: 'absolute',
        ...
        >
          <Button className="rg-btn-2 justify-center" style={{ flex: 1 }} onPress={onClose}>
            Close
          </Button>
          <Button
            ...
            startContent={<Icon name="x" size={15} stroke={2.4} />}
          >
            {readOnly
              ? 'Read-only mode'
              : p.status === 'revoked'
    ...
    >
      ...
      >
          ...
          >
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                ...
                {data && (
                  <span className={`badge ${data.success ? 'badge-safe' : 'badge-warn'}`}>
                ...
                {!data && !err && (
                  <span className="badge badge-neutral">
                    <span className="dot"></span>Loading…
                  </span>
                )}
                {err && (
                  <span className="badge badge-warn">
                    <span className="dot"></span>Unavailable
                  </span>
                )}
              </div>
        ...
        <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 22 }}>
          {!data && !err && (
            <div style={{ fontSize: 12.5, color: 'var(--t2)' }}>
              Decoding transaction from chain…
            </div>
          )}
          {err && (
            <div
            ...
            >
              ...
              <div style={{ fontSize: 11.5, color: 'var(--t2)' }}>
                {err}. It may be older than the fullnode's retention window, or a demo record. You
                can still open it on the explorer below.
              </div>
            </div>
          )}
          {data && (
            <>
              <div>
                ...
                <div>
                  {row(
                    'Executed by',
                    <span className="mono" style={{ color: 'var(--sui)' }}>
                      {short(data.sender)}
                    </span>
                  )}
                  ...
                  {row(
                    'Gas paid by',
                    data.gasOwner === data.sender ? 'signer (self)' : short(data.gasOwner)
                  )}
                  {row(
                    'Timestamp',
                    data.timestampMs
                      ? new Date(data.timestampMs).toISOString().replace('T', ' ').slice(0, 19) +
                          ' UTC'
                      : '—'
                  )}
                </div>
              ...
              <div>
                <div className="eyebrow" style={{ marginBottom: 10 }}>
                  Programmable transaction block · {data.calls.length} call
                  {data.calls.length === 1 ? '' : 's'}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {data.calls.length === 0 && (
                    <div style={{ fontSize: 12, color: 'var(--t2)' }}>
                      No Move calls in this transaction.
                    </div>
                  )}
                  {data.calls.map((c, i) => (
                    <div
                    ...
                    >
                      ...
                      <span
                        className="mono"
                        style={{ fontSize: 11.5, color: 'var(--t0)', wordBreak: 'break-all' }}
                      >
                        {c.target}
                      </span>
                    </div>
              ...
              <div>
                <div className="eyebrow" style={{ marginBottom: 10 }}>
                  Events emitted · {data.events.length}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {data.events.length === 0 && (
                    <div style={{ fontSize: 12, color: 'var(--t2)' }}>No events emitted.</div>
                  )}
                  {data.events.map((e, i) => (
                    <div
                      key={i}
                    ...
                    {data.balanceChanges.map((b, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {up ? '+' : ''}
                            {amt.toLocaleString(undefined, { maximumFractionDigits: dec })} {sym}
                          </span>
                        </div>
                      );
                    })}
          ...
          <Button
            ...
            onPress={() => window.open(explorer, '_blank', 'noopener,noreferrer')}
            startContent={<Icon name="link" size={14} />}
          >
            View on SuiScan
          </Button>
        </div>
