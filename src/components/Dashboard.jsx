  ...
  return (
    <Card className="overflow-hidden border border-[color:var(--border)] bg-[color:var(--glass)] px-[18px] py-4">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <div style={{ color: accent || 'var(--t2)', opacity: 0.9 }}>
          <Icon name={icon} size={16} />
        </div>
      </div>
      <div
        className="mono display"
        style={{ fontSize: 26, fontWeight: 600, marginTop: 10, letterSpacing: '-0.02em' }}
      >
        {value}
      </div>
      <div
...
function PriceChart({ data, crashState }) {
  ...
  const referenceLine = showRef
    ? { value: Number((data[0] * 0.916).toFixed(dec)), label: 'trigger −8.4%' }
    : undefined;
  return (
...
export function Dashboard({ state, live }) {
  ...
  useEffect(() => {
    ...
    let alive = true;
    getSuiPriceHistory(14)
      .then((h) => {
        if (alive) setPriceHist(h.status === 'ok' && h.prices.length > 2 ? h.prices : null);
      })
      .catch(() => {});
    return () => {
      alive = false;
  ...
  const dailyRet = priceHist
    ? priceHist.slice(1).map((px, i) => (px - priceHist[i]) / priceHist[i])
    : [];
  const vol = dailyRet.length
    ? Math.sqrt(dailyRet.reduce((s, r) => s + r * r, 0) / dailyRet.length) * 100
    : null;
  ...
  const riskSeries = priceHist
    ...
    : null;
  const shownRisk = liveOn
    ? Math.min(95, Math.round(15 + util * 50 + conc * 30 + volContribution))
    : risk;
  const liveSpark = live?.market?.sui_spark;
  const chartData =
    liveOn && Array.isArray(liveSpark) && liveSpark.length >= 2 ? liveSpark : suiSpark;
  ...
  const gaugeFactors = liveOn
    ...
    : [
        {
          k: 'Volatility (1h)',
          v: crashState === 'crashing' ? 'extreme' : 'moderate',
          lv: crashState === 'crashing' ? 'danger' : 'warn',
        },
        { k: 'Slippage headroom', v: 'within cap', lv: 'safe' },
        {
          k: 'Pool liquidity',
          v: crashState === 'crashing' ? 'thinning' : 'healthy',
          lv: crashState === 'crashing' ? 'warn' : 'safe',
        },
      ];
  const liveReasoning = liveOn
    ? {
        ...
        factors: [
          {
            k: 'Budget utilisation',
            w: Math.round(util * 50),
            lv: util > 0.7 ? 'danger' : util > 0.4 ? 'warn' : 'safe',
          },
          { k: 'Concentration', w: Math.round(conc * 30), lv: conc > 0.5 ? 'warn' : 'safe' },
          {
            k: 'Market volatility',
            w: Math.round(volContribution),
            lv: (vol || 0) > 4 ? 'danger' : (vol || 0) > 2 ? 'warn' : 'safe',
          },
  ...
  const banner =
    crashState === 'crashing'
      ? {
          ...
          bg: 'var(--danger-dim)',
          icon: 'alert',
          t: 'Flash crash detected · SUI −8.4% in 6 min',
          s: 'Risk score spiking — agent evaluating rescue conditions…',
        }
      : crashState === 'rescuing'
        ? {
            c: 'var(--accent)',
            bg: 'var(--accent-dim)',
            icon: 'bolt',
            t: 'Agent autonomously executing rescue grid',
            s: 'Placing limit orders on Deepbook within policy budget — no signature required.',
          }
        : crashState === 'rescued'
          ? {
              c: 'var(--safe)',
              bg: 'var(--safe-dim)',
              icon: 'check',
              t: 'Rescue complete · 2 rungs filled, budget intact',
              s: 'Agent bought the dip inside policy limits and logged every action on-chain.',
            }
          : null;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      {banner && (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div className="display" style={{ fontWeight: 600, fontSize: 14.5, color: banner.c }}>
              {banner.t}
            </div>
      ...
      <div className="rg-kpi">
        {liveOn ? (
          <StatCard
            ...
            value={`$${usd6(sum?.total_authorized)}`}
            icon="wallet"
            sub={{
              text: `${sum?.active_policies ?? 0} active polic${(sum?.active_policies ?? 0) === 1 ? 'y' : 'ies'}`,
              color: 'var(--t1)',
            }}
          />
        ) : (
          <StatCard
            ...
            value={`$${fmtUsd(animTotal)}`}
            icon="wallet"
            sub={{
              text: `${p.chg24h > 0 ? '+' : ''}${p.chg24h}% · 24h`,
              color: p.chg24h < 0 ? 'var(--danger)' : 'var(--safe)',
            }}
          />
        )}
        {liveOn ? (
        ...
        <StatCard
          label="Agent status"
          value={agentOn ? 'Autonomous' : 'Paused'}
          icon="bolt"
        ...
        />
        <StatCard
          label={liveOn ? 'Risk score · advisory' : 'Risk score'}
          value={Math.round(shownRisk)}
          icon="shield"
          accent={
            shownRisk >= 70 ? 'var(--danger)' : shownRisk >= 45 ? 'var(--warn)' : 'var(--safe)'
          }
          spark={liveOn ? riskSeries || undefined : RG.riskHistory}
          sparkColor={
            shownRisk >= 70 ? 'var(--danger)' : shownRisk >= 45 ? 'var(--warn)' : 'var(--safe)'
          }
          sub={{
            text: shownRisk >= 70 ? 'critical' : shownRisk >= 45 ? 'elevated' : 'stable',
            color:
              shownRisk >= 70 ? 'var(--danger)' : shownRisk >= 45 ? 'var(--warn)' : 'var(--safe)',
          }}
        />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11.5, color: 'var(--t2)' }}>
                    {liveOn ? 'Deepbook v3 · live indexer' : 'Deepbook v3 · price via Pyth'}
                  </div>
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div
                  ...
                  style={{
                    fontSize: 22,
                    fontWeight: 600,
                    color: crashState === 'crashing' ? 'var(--danger)' : 'var(--t0)',
                  }}
                >
                ...
                <div
                  ...
                  style={{
                    ...
                    fontWeight: 600,
                    color:
                      (liveOn ? (liveChg ?? 0) : RG.prices.SUI.chg) < 0
                        ? 'var(--danger)'
                        : 'var(--safe)',
                  }}
                >
          ...
          <div className="card">
            <div className="card-hd" style={{ paddingBottom: 12 }}>
              ...
              <div className="badge badge-neutral">
                {liveOn
                  ? `${sum?.positions?.length ?? 0} on-chain`
                  : `${RG.positions.length} active`}
              </div>
            </div>
            {liveOn ? (
              <div style={{ padding: '0 6px 8px' }}>
                {(sum?.positions?.length ?? 0) === 0 && (
                  <div style={{ padding: '14px 12px', fontSize: 12.5, color: 'var(--t2)' }}>
                    No policies yet — create a strategy.
                  </div>
                )}
                  ...
                  <tbody>
                    {(sum?.positions ?? []).map((po, i) => {
                      const used = Math.round(
                        (Number(po.spent_amount) / Number(po.budget_ceiling || 1)) * 100
                      );
                      return (
                        <tr
                          key={po.wrapper_id}
                        ...
                        >
                          <td style={{ padding: '11px 12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              <span
                                className="mono"
                                style={{ fontSize: 11.5, color: 'var(--sui)' }}
                              >
                                {po.wrapper_id.slice(0, 6)}…{po.wrapper_id.slice(-4)}
                              </span>
                            </div>
                          </td>
                          <td style={{ padding: '11px 12px', textAlign: 'right' }} className="mono">
                            {fmtUsd(Number(po.spent_amount) / 1e6, 0)} /{' '}
                            {fmtUsd(Number(po.budget_ceiling) / 1e6, 0)} USDC
                          </td>
                          <td style={{ padding: '11px 12px', textAlign: 'right' }}>
                            <span
                              className={`badge ${po.status === 'active' ? 'badge-safe' : 'badge-warn'}`}
                            >
                              <span
                                className={`dot ${po.status === 'active' ? 'pulse' : ''}`}
                              ></span>
                              {po.status} · {used}%
                            </span>
                          </td>
                  ...
                  <tbody>
                    {RG.positions.map((po, i) => (
                      <tr key={i} style={{ borderTop: '1px solid var(--border)' }}>
                        ...
                        <td style={{ padding: '11px 12px', color: 'var(--t1)', fontSize: 12.5 }}>
                          {po.side}
                        </td>
                        <td style={{ padding: '11px 12px', textAlign: 'right' }} className="mono">
                          ${fmtUsd(po.size, 0)}
                        ...
                        <td style={{ padding: '11px 12px', textAlign: 'right' }} className="mono">
                          <div
                            style={{
                              color: po.pnl < 0 ? 'var(--danger)' : 'var(--safe)',
                              fontWeight: 600,
                            }}
                          >
                        ...
                        <td style={{ padding: '11px 12px', textAlign: 'right' }}>
                          <span
                            className={`badge ${po.risk === 'low' ? 'badge-safe' : po.risk === 'med' ? 'badge-warn' : 'badge-danger'}`}
                          >
                            {po.risk}
                          </span>
                        </td>
            ...
            >
              {gaugeFactors.map((r) => (
                ...
                >
                  ...
                  <span className={`badge badge-${r.lv}`}>
          ...
          <div className="card">
            ...
            >
              {liveOn && feed.length === 0 && (
                <div style={{ padding: '18px', fontSize: 12, color: 'var(--t2)' }}>
              ...
              {feed.slice(0, 6).map((a, i) => (
                <div
                  key={a.t + i}
                  className={i === 0 && crashState === 'rescuing' ? 'fade-up' : ''}
                  style={{
                    display: 'flex',
                    gap: 11,
                    padding: '10px 18px',
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--t0)' }}>
                      {a.title}
                    </div>
                    <div style={{ fontSize: 11, color: 'var(--t2)' }}>
                      {a.t} · {a.policy}
    ...
    <div className="card">
      <div className="card-hd" style={{ paddingBottom: 12 }}>
        ...
        <span className="badge badge-neutral" style={{ fontSize: 9.5 }}>
          <Icon name={mode === 'local' ? 'cpu' : 'cloud'} size={10} />
          {mode === 'local' ? 'on-device' : 'cloud'}
        </span>
      </div>
          ...
          {R.factors.map((f) => (
            <div key={f.k}>
              ...
              >
                ...
                <span className="mono" style={{ color: lvCol[f.lv], fontWeight: 600 }}>
                  +{f.w}
                </span>
              </div>
              <div
              ...
              >
                <div
                  style={{
                    ...
                    height: '100%',
                    borderRadius: 100,
                    background: lvCol[f.lv],
                    transition: 'width .5s ease',
                    boxShadow: `0 0 8px ${lvCol[f.lv]}`,
                  }}
                />
              </div>
            </div>
