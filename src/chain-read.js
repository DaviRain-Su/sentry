...
async function policyEvents() {
  ...
  for (let i = 0; i < 5; i++) {
    const res = await rpc('suix_queryEvents', [
      { MoveEventModule: { package: RG.package_id, module: 'policy' } },
      cursor,
      50,
      true,
    ]);
...
export async function getMarket() {
  try {
    ...
    const pick = (k) =>
      t[k] ? { last_price: t[k].last_price, base_volume: t[k].base_volume } : null;
    let sui_spark = [],
      sui_change = null;
    try {
      ...
      sui_spark = prices;
      if (prices.length >= 2)
        sui_change = +(((prices[prices.length - 1] - prices[0]) / prices[0]) * 100).toFixed(2);
    } catch {
      /* spark optional */
    }
    return {
      status: 'ok',
      market: {
        SUI_DBUSDC: pick('SUI_DBUSDC'),
        DEEP_DBUSDC: pick('DEEP_DBUSDC'),
        WAL_DBUSDC: pick('WAL_DBUSDC'),
        sui_spark,
        sui_change,
      },
    };
...
export async function getBalances(owner) {
  ...
  const price = {
    SUI: Number(mk.SUI_DBUSDC?.last_price) || 0,
    USDC: 1,
    DEEP: Number(mk.DEEP_DBUSDC?.last_price) || 0,
    WAL: Number(mk.WAL_DBUSDC?.last_price) || 0,
  };
  const sym = (ct) =>
    ct.endsWith('::sui::SUI')
      ? ['SUI', 9]
      : ct.includes('::DBUSDC::DBUSDC')
        ? ['USDC', 6]
        : ct.endsWith('::deep::DEEP')
          ? ['DEEP', 6]
          : ct.endsWith('::wal::WAL')
            ? ['WAL', 9]
            : null;
...
export async function listPolicies(owner) {
  ...
  const policies = ev
    ...
    .map((e) => ({
      wrapper_id: e.parsedJson.wrapper_id,
      mandate_id: e.parsedJson.mandate_id,
      owner: e.parsedJson.owner,
      pool_id: e.parsedJson.pool_id,
      budget_ceiling: String(e.parsedJson.budget_ceiling),
      max_slippage_bps: Number(e.parsedJson.max_slippage_bps),
  ...
  for (const e of ev) {
    ...
    if (type === 'PolicyCreated')
      ...
      activity.push({
        ...
        detail: `Spent ${Number(pj.quote_amount_spent) / 1e6} USDC · slippage ${pj.slippage_bps}bps`,
...
export async function getSummary(owner) {
  ...
  for (const p of policies) {
    ...
    const status = m?.revoked
      ? 'revoked'
      : m && Date.now() >= Number(m.expires_at_ms)
        ? 'expired'
        : 'active';
    positions.push({
      wrapper_id: p.wrapper_id,
      pool_id: w.pool_id,
      budget_ceiling: w.budget_ceiling,
...
export async function getTransaction(digest) {
  try {
    ...
    const g = fx.gasUsed || {};
    const gasMist =
      Number(g.computationCost || 0) + Number(g.storageCost || 0) - Number(g.storageRebate || 0);
    ...
    const balanceChanges = (r.balanceChanges || []).map((b) => ({
      coinType: b.coinType,
      amount: b.amount,
      owner: b.owner?.AddressOwner || null,
    }));
    return {
      status: 'ok',
      tx: {
...
export async function getSuiPriceHistory(days = 30) {
  try {
    ...
    return prices.length > 2
      ? { status: 'ok', prices }
      : { status: 'error', message: 'insufficient DeepBook history' };
  } catch (e) {
    return { status: 'error', message: String(e?.message || e) };
  }
}
