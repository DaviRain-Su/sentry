...
export function attachMarketData(RG) {
  ...
  RG.signers = [
    ...
    {
      name: 'Local OWS signer',
      kind: 'local',
      status: 'offline',
      detail: '~/.ows vault · setup required',
    },
    {
      name: 'Remote Worker executor',
      kind: 'cloud',
      status: 'ok',
      detail: 'Sui Testnet demo · no owner keys',
    },
  ];
  ...
  RG.dataFeeds = [
    ...
    },
    {
      ...
      provider: 'Local Agent + OWS',
      endpoint: 'localhost daemon + ~/.ows',
      type: 'local',
      access: 'proxy',
      cadence: 'on demand',
  ...
  RG.runtimes = {
    cloud: {
      ...
      uptime: '14d 06h',
      llm: 'server-side parser · no wallet custody',
      heartbeat: '2s ago',
      loopMs: 850,
      tick: 'every 8s',
      watching: 3,
      gas: { station: 'Sentry Gas Station', bal: 4.81, unit: 'SUI' },
      privacy: 'Remote mode keeps the Sui Testnet demo alive, but production signing moves local.',
      tags: ['testnet', 'always-on', 'optional'],
      health: [
        ...
        { k: 'Owner custody', v: 'no owner key in Worker', ok: true },
      ],
      log: [
    ...
    local: {
      ...
      icon: 'cpu',
      status: 'offline',
      host: 'Your machine · daemon + OWS vault',
      ...
      watching: 0,
      gas: { station: 'Sentry Gas Station', bal: 4.81, unit: 'SUI' },
      privacy: 'Decision logic, wallet signing, exchange API keys and asset inventory stay local.',
      tags: ['default', 'OWS vault', 'trade keys local'],
      health: [
        { k: 'Daemon process', v: 'not running', ok: false },
        { k: 'OWS vault', v: '~/.ows expected', ok: null },
        { k: 'Secret store', v: 'OS keychain + ~/.sentry', ok: null },
        { k: 'Exchange API keys', v: 'read + trade only', ok: null },
      ],
      log: [
        { t: '—', d: 'Daemon offline · no local signing loop is running yet' },
        { t: 'setup', d: 'Run: sentry agent init --vault ~/.ows, then sentry agent run' },
      ],
    },
  };
