...
const CRASH_TAIL = [4.1, 3.96, 3.84, 3.71, 3.79];
...
function NavItem({ icon, label, active, onClick, badge, badgeClass = 'badge-accent' }) {
  return (
    <button onClick={onClick} className={`rg-navitem${active ? ' is-active' : ''}`}>
      ...
      {badge != null && (
        <span className={`badge ${badgeClass}`} style={{ fontSize: 9.5, padding: '2px 7px' }}>
          {badge}
        </span>
      )}
    </button>
...
function buildSourceMeta({ live, loading, readOnly, meta, count }) {
  if (!live) {
    return {
      ...
      tone: 'neutral',
      detail:
        'Local sample policies and activity for walkthroughs. These rows are not on-chain executions.',
    };
  }
...
export default function App({ onExit }) {
  ...
  const liveProfileLoading =
    liveReadsEnabled &&
    (!!liveDashboardQuery.loading?.balances || !!liveDashboardQuery.loading?.summary);
  const liveReadMeta = liveDashboardQuery.isError
    ? {
        source: 'error',
        error: String(liveDashboardQuery.error?.message || liveDashboardQuery.error),
      }
    : liveDashboard.meta;
  const refreshLivePolicies = () =>
  ...
  useEffect(() => {
    const h = (e) => {
      setRuntimeMode(e.detail || null);
      setRuntimeOpen(true);
    };
    window.addEventListener('rg:runtime', h);
    return () => window.removeEventListener('rg:runtime', h);
  }, []);
  const pushNotif = (kind, title) =>
    setNotifs((n) => [{ id: ++notifId.current, kind, title, time: 'now', read: false }, ...n]);
  ...
  const simulateCrash = () => {
      ...
      setActivity((a) => [
        {
          ...
          title: 'Trigger condition met · SUI −8.4%',
          detail: 'Reference −8% breached. Agent authorized by policy to deploy rescue grid.',
          amount: 0,
          tx: null,
          risk: 82,
          mode,
      ...
      setActivity((a) => [
        {
          ...
          kind: 'exec',
          policy: 'SUI Crash Rescue Grid',
          title: 'Rung #1 partial fill · 14.3 SUI @ 3.85',
          detail: 'Book thinned mid-fill (60%) · agent re-quoting remainder on Deepbook',
          amount: -55.0,
          tx: '0xe1f4…7c33',
          risk: 70,
          mode,
      ...
      setActivity((a) => [
        {
          ...
          kind: 'retry',
          policy: 'SUI Crash Rescue Grid',
          title: 'Rung #1 remainder filled on retry · 9.6 SUI @ 3.86',
          detail: 'Re-quote succeeded · slippage 0.8% · budget check passed',
          amount: -37.0,
          tx: '0x3da9…6b22',
          risk: 62,
          mode,
      ...
      setActivity((a) => [
        {
          ...
          kind: 'exec',
          policy: 'SUI Crash Rescue Grid',
          title: 'Bought 24.6 SUI @ 3.74',
          detail: 'Rescue rung #2 filled on Deepbook · slippage 0.9% · 184 / 500 USDC used',
          amount: -92.0,
          tx: '0x9a02…b418',
          risk: 58,
          mode,
    ...
    after(6000, () => {
      setActivity((a) => [
        {
          ...
          kind: 'policy',
          policy: 'SUI Crash Rescue Grid',
          title: 'Activity logged on-chain',
          detail: 'Agent execution record committed · 2 fills · 184 USDC spent of 500 cap',
          amount: 0,
          tx: '0x55cd…2e19',
          risk: null,
          mode,
      ...
      setPolicies((ps) =>
        ps.map((p) =>
          p.strategy === 'rescue-grid' ? { ...p, budgetUsed: 184, execs: p.execs + 2 } : p
        )
      );
  ...
  const resetDemo = () => {
    ...
    setActivity(RG.activity);
    setPolicies(RG.policies);
  };

  useEffect(() => {
    if (!liveReadsEnabled || !authed) return;
    if (!livePoliciesLoading && liveDashboardQuery.data)
      setPolicies(liveDashboardQuery.data.policies);
    else if (liveDashboardQuery.isError) setPolicies([]);
  }, [
    liveReadsEnabled,
    authed,
    liveDashboardQuery.data,
    liveDashboardQuery.isError,
    livePoliciesLoading,
  ]);
  ...
  const deployLiveMutation = useMutation({
    mutationFn: async ({ text, meta }) => {
      const parsed = await parseIntent(
        owner,
        text || `When SUI drops more than 8%, deploy a ${meta.budget} USDC rescue grid`
      );
  ...
  const togglePolicy = (p) => {
    ...
    showToast(
      p.status === 'active'
        ? 'Strategy paused — agent holds, no new actions'
        : 'Strategy resumed — agent active within policy',
      p.status === 'active' ? 'var(--warn)' : 'var(--accent)'
    );
  };
    ...
    setActivity((a) => [
      {
        ...
        kind: 'rebalance',
        policy: p.name,
        title: 'Manual rebalance triggered',
        detail:
          'Owner requested re-centering · agent restored target exposure within policy limits.',
        amount: 0,
        tx:
          '0x' +
    ...
    setActivity((a) => [
      {
        ...
        kind: 'guardian',
        policy: 'All policies',
        title: 'Emergency stop triggered by owner',
        detail:
          'Global circuit breaker engaged · every agent policy frozen on-chain · no further execution permitted',
        amount: 0,
        tx: '0xkill…stop',
        risk: null,
        mode,
  ...
  const deployPolicy = (
    ...
    if (liveMode) {
      if (!WORKER_CONFIGURED) {
        showToast(
          'On-chain deploy needs the Worker — set VITE_WORKER_URL and run it',
          'var(--warn)'
        );
        return;
      }
      deployLive(text, meta);
      return;
    }
    if (readOnlyLiveMode) {
      showToast(
        'Worker preview is read-only — connect a Sui wallet to sign and deploy',
        'var(--warn)'
      );
      return;
    }
    const np = {
      ...
      status: 'active',
      mode,
      budgetCap: meta.budget,
      budgetUsed: 0,
      scope: [meta.scope],
      maxSlippage: meta.slip,
    ...
    const finish = () => {
      ...
      try {
        Object.keys(localStorage).forEach((k) => {
          if (/dapp-kit/i.test(k)) localStorage.removeItem(k);
        });
      } catch {
        /* ignore */
      }
      ...
      <div style={{ display: 'flex', height: '100vh', position: 'relative', zIndex: 1 }}>
        ...
        >
          <div className="rg-sidebar-scroll">
            <div style={{ padding: '0 8px 8px', cursor: 'pointer' }} onClick={onExit}>
              <Logo />
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3, marginTop: 18 }}>
              <div className="eyebrow" style={{ padding: '0 13px 8px' }}>
                Workspace
              </div>
              <NavItem
                icon="dashboard"
              ...
              <NavItem
                ...
                active={view === 'strategies' || view === 'strategy-detail'}
            ...
            <div className="card rg-agentcard" style={{ padding: 14, marginBottom: 12 }}>
              ...
              >
                ...
                <button
                  aria-label={agentOn ? 'Disable agent' : 'Enable agent'}
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    position: 'relative',
                    background: agentOn ? 'var(--accent)' : 'var(--glass-hi)',
                    transition: 'all .18s',
                  }}
                >
                  <div
              ...
              >
                {[
                  ...
                  <button
                    key={m.id}
                    title={mode === m.id ? 'View agent runtime' : `Switch to ${m.l}`}
                    onClick={() => {
                      if (mode === m.id) {
                        setRuntimeMode(m.id);
                        setRuntimeOpen(true);
                    ...
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 600,
                      background: mode === m.id ? 'var(--glass-hi)' : 'transparent',
                      color: mode === m.id ? 'var(--accent)' : 'var(--t2)',
                      transition: 'all .14s',
                    }}
                  >
              ...
              <button
                ...
                className="rg-navlabel"
                title="View agent runtime details"
                style={{
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'space-between',
              ...
              >
                ...
                <div style={{ fontSize: 10.5, lineHeight: 1.45, color: 'var(--t1)' }}>
                  Gas is <strong style={{ color: 'var(--t0)' }}>sponsored</strong> — the agent pays
                  fees from a gas station, so it acts without SUI of its own.
                </div>
              </div>
            </div>
            ...
            {halted ? (
              <Button
                className="mb-3 bg-accent text-accent-foreground font-semibold"
                fullWidth
                onPress={resumeAgents}
              ...
              <Button
                className="mb-3 bg-danger text-white font-semibold"
                fullWidth
        ...
        <main style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          ...
          >
            <div style={{ flex: 1 }}>
              ...
              <div className="rg-subtitle" style={{ fontSize: 11.5, color: 'var(--t2)' }}>
                {titles[view].s}
              </div>
            </div>
                    ...
                    <div>
                      ...
                      <div
                        ...
                        style={{
                          fontSize: 10,
                          fontWeight: 600,
                          color: chg < 0 ? 'var(--danger)' : 'var(--safe)',
                        }}
                      >
                        {chg > 0 ? '+' : ''}
                        {chg}%
            ...
            <div style={{ position: 'relative' }}>
              <Button
                ...
                className="relative rg-btn-ghost"
                onPress={() => {
                  setNotifOpen((o) => !o);
                  if (!notifOpen) setNotifs((n) => n.map((x) => ({ ...x, read: true })));
                }}
              >
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '24px 26px 60px' }}>
            {liveReadsEnabled && (
              ...
              >
                <span
                  style={{
                    color: sourceMeta.tone === 'warn' ? 'var(--warn)' : 'var(--accent)',
                    flexShrink: 0,
                  }}
                >
                  <Icon name={sourceMeta.icon} size={16} />
                </span>
                <div style={{ flex: 1, fontSize: 12.5, color: 'var(--t1)', lineHeight: 1.45 }}>
                  <strong style={{ color: 'var(--t0)' }}>{sourceMeta.title}</strong> for{' '}
                  {ownerShort}. {sourceMeta.detail}
                  {sourceMeta.error && (
                    <span className="mono" style={{ color: 'var(--warn)', marginLeft: 8 }}>
                      worker_error={sourceMeta.error.slice(0, 90)}
                    </span>
                  )}
                </div>
            ...
            {halted && (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div
                    className="display"
                    style={{ fontWeight: 600, fontSize: 14.5, color: 'var(--danger)' }}
                  >
                    Circuit breaker engaged — all agents halted
                  </div>
                  <div style={{ fontSize: 12.5, color: 'var(--t1)', marginTop: 1 }}>
                    Every policy is frozen on-chain. No agent can execute until you resume — your
                    funds and limits are untouched.
                  </div>
                </div>
                <Button
                  size="sm"
            ...
            {view === 'dashboard' && (
              <Dashboard
                ...
                live={
                  liveReadsEnabled
                    ? { summary: liveSummary, market: liveMarket, activity: liveActivity }
                    : null
                }
              />
            )}
            ...
            {view === 'data' && (
              <DataSources onToast={showToast} live={liveFeed} setLive={setLiveFeed} />
            )}
            {view === 'active' &&
              (displayedPolicies.find((x) => x.id === liveId) ? (
                <ActiveStrategy
                  p={displayedPolicies.find((x) => x.id === liveId)}
                  activity={shownActivity}
                  onBack={() => setView('policies')}
                  onToggle={togglePolicy}
                  onRebalance={rebalanceNow}
                ...
                <PoliciesView
                  ...
                  onLive={(p) => openLivePolicy(p.id)}
                  live={liveReadsEnabled}
                  readOnly={readOnlyLiveMode}
                  loading={livePoliciesLoading}
                  source={sourceMeta}
                />
              ))}
            {view === 'policies' && (
        ...
        {toast && (
          <div
            ...
            style={{
              ...
              borderRadius: 'var(--r-lg)',
              background: 'var(--bg-3)',
              border: `1px solid ${toast.c}`,
              boxShadow: `0 12px 40px -10px ${toast.c}80`,
            }}
          >
            <span style={{ color: toast.c }}>
        ...
        <TweaksPanel>
          ...
          <TweakRadio
            ...
            value={t.crashSeverity}
            options={['mild', 'severe']}
            onChange={(v) => setTweak('crashSeverity', v)}
          />
          <TweakToggle
            label="Live market jitter"
            value={t.liveJitter}
