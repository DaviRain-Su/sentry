export const DEFAULT_OKX_RATE_LIMIT_POLICY = {
  max_attempts: 3,
  base_backoff_ms: 250,
  max_backoff_ms: 2_000,
  min_interval_ms: 100,
  retry_http_statuses: [429, 500, 502, 503, 504],
  retry_okx_codes: ['50011'],
};
...
function policyWithDefaults(policy = {}) {
  return {
    ...DEFAULT_OKX_RATE_LIMIT_POLICY,
    ...policy,
    retry_http_statuses:
      policy.retry_http_statuses || DEFAULT_OKX_RATE_LIMIT_POLICY.retry_http_statuses,
    retry_okx_codes: policy.retry_okx_codes || DEFAULT_OKX_RATE_LIMIT_POLICY.retry_okx_codes,
  };
}
...
export function okxBackoffDelayMs({ attempt, response, policy = {}, nowMs = Date.now() } = {}) {
  const merged = policyWithDefaults(policy);
  const retryAfter = parseRetryAfterMs(headerValue(response?.headers, 'retry-after'), nowMs);
...
export function isRetryableOkxResult({ response, body, error, policy = {} } = {}) {
  const merged = policyWithDefaults(policy);
  if (error) return true;
  if (response && merged.retry_http_statuses.includes(Number(response.status))) return true;
  if (body?.code && merged.retry_okx_codes.includes(String(body.code))) return true;
  return false;
}

export function createOkxRateLimiter(options = {}) {
  const { policy = {}, nowMs = () => Date.now(), sleep = defaultSleep } = options;
  const merged = policyWithDefaults(policy);
  const lastByBucket = new Map();

  return {
    async wait(bucket = 'okx:default') {
...
export async function fetchOkxJsonWithBackoff(options = {}) {
  const {
    fetchOnce,
    policy = {},
    sleep = defaultSleep,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    const retryable = isRetryableOkxResult({ ...result, policy: merged });
    ...
    const delay_ms = okxBackoffDelayMs({
      attempt,
    ...
    retry_events.push({
      ...
      okx_code: result.body?.code ?? null,
...
export function okxRetrySummary(result = {}) {
  return {
