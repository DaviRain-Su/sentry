export const DEFAULT_CHAIN_RPC_RETRY_POLICY = {
  max_attempts: 3,
  base_backoff_ms: 250,
  max_backoff_ms: 2_000,
  min_interval_ms: 50,
  retry_http_statuses: [408, 425, 429, 500, 502, 503, 504],
  retry_rpc_codes: [-32005, -32004, -32002, -32603],
};
...
function policyWithDefaults(policy = {}) {
  return {
    ...DEFAULT_CHAIN_RPC_RETRY_POLICY,
    ...policy,
    retry_http_statuses:
      policy.retry_http_statuses || DEFAULT_CHAIN_RPC_RETRY_POLICY.retry_http_statuses,
    retry_rpc_codes: policy.retry_rpc_codes || DEFAULT_CHAIN_RPC_RETRY_POLICY.retry_rpc_codes,
  };
}
...
export function parseChainRpcRetryAfterMs(value, nowMs = Date.now()) {
...
export function chainRpcBackoffDelayMs({
  attempt,
  response,
  policy = {},
  nowMs = Date.now(),
  ...
  const retryAfter = parseChainRpcRetryAfterMs(
...
export function isRetryableChainRpcResult({ response, body, error, policy = {} } = {}) {
  const merged = policyWithDefaults(policy);
  if (error) return true;
  if (response && merged.retry_http_statuses.includes(Number(response.status))) return true;
  if (body?.error?.code !== undefined) {
    return merged.retry_rpc_codes.includes(Number(body.error.code));
  }
  return false;
}

export function createChainRpcRateLimiter(options = {}) {
  const { policy = {}, nowMs = () => Date.now(), sleep = defaultSleep } = options;
  const merged = policyWithDefaults(policy);
  const lastByBucket = new Map();

  return {
    async wait(bucket = 'chain-rpc:default') {
...
export async function fetchChainRpcJsonWithBackoff(options = {}) {
  const {
    fetchOnce,
    policy = {},
    sleep = defaultSleep,
    rateLimiter = null,
    bucket = 'chain-rpc:default',
    nowMs = () => Date.now(),
  } = options;
  const merged = policyWithDefaults(policy);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    try {
      ...
      result = { error, response: null, body: null };
    }

    const retryable = isRetryableChainRpcResult({ ...result, policy: merged });
    ...
    const delay_ms = chainRpcBackoffDelayMs({
      attempt,
    ...
    retry_events.push({
      ...
      rpc_code: result.body?.error?.code ?? null,
...
export function chainRpcRetrySummary(result = {}) {
  return {
